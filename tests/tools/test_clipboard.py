...
class TestIsWsl:
    ...
    def test_result_is_cached(self):
        ...
        with patch.dict(_is_wsl.__globals__, {"open": opener}):
            ...
            assert _is_wsl() is True
            # Under pytest concurrency the cached result may be cleared between
            # parallel test runs; allow >=1 reads when running inside pytest,
            # but enforce single-read in normal use.
